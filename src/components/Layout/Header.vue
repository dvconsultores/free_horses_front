<template>
  ...
  >
    <v-row>
      <v-col cols="11" class="space align paddbottomdel" style="display: flex">
        <aside class="contleft divrow acenter">
          ...
          <a class="eliminarmobile" href="#" @click="$router.push('/')">
            <img class="logo" :src="`${$store.state.baseURL}themes/${$store.state.theme}/logo.svg`" alt="Logo">
          </a>
        </aside>
        ...
        <v-col class="jend padd eliminarmobile" style="display: flex">
          ...
          <v-col class="contmiddle end padd">
            <a v-for="(item, index) in dataHeader" :key="index" :href="item.link"
              class="h9-em openMenuCollections" @click="$router.push('/')">
              {{ item.title }}
              <!-- <span class="mask"><span class="notdefault-clr">{{ item.title }}</span></span>
              <span class="mask"><span class="notdefault-clr">{{ item.title }}</span></span> -->
            </a>
          </v-col>
        </v-col>
